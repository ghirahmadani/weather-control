...
const Main = () => {
  ...
  const onSearch = async (event) => {
    ...
    await axios
      .get(`${process.env.REACT_APP_WEATHER_URL}weather?q=${city}&appid=${process.env.REACT_APP_WEATHER_KEY}`)
      .then((res) => {
        setWeather(res.data);
        setCity('')
    ...
    if (navigator.geolocation){
      await navigator.geolocation.getCurrentPosition(
        function(position){
          ...
          axios
          .get(`${process.env.REACT_APP_WEATHER_URL}weather?lat=${lat}&lon=${lon}&appid=${process.env.REACT_APP_WEATHER_KEY}`)
          .then((res) => {
            setWeather(res.data);
            setCity('')
            setErrorLocation(null);
  ...
  return (
    <div className="lg:container w-full">
      <div className="flex flex-col justify-center items-center w-full h-full gap-6">
        ...
        <div
          className="fixed inset-x-0 mx-auto top-[100px] w-10/12 rounded-lg bg-white border text-red-700 px-4 py-3"
          role="alert"
        >
        ...
        <p className="text-base text-[#323368]">
